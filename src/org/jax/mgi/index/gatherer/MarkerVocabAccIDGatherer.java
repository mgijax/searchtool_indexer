...
/**
 * Gather up all of the information needed to search for markers, by vocabulary accession ID.
 *  
 * @author mhall
 *
 */
...
public class MarkerVocabAccIDGatherer extends AbstractGatherer {
    ...
    private Logger log = Logger.getLogger(MarkerVocabAccIDGatherer.class.getName());
    
    private HashMap<String, String>    hm        = new HashMap<String, String>();

    public MarkerVocabAccIDGatherer(Configuration config) {
        super(config);
    ...
    private void doVocabAccessionID() throws SQLException, InterruptedException {
        ...
        // As long as the column names remain the same, the SQL for any given vocabulary
        // can move independently of each other.

        log.info("Collecting GO Accession ID's");
        
        String GO_ACCID_KEY = "select tv._Term_key, tv.accId, tv.vocabName, tv.term"
        ...
        String INTERPRO_ACCID_KEY = "select tv._Term_key, tv.accId, tv.vocabName, tv.term"
                + " from VOC_Term_View tv, VOC_Annot_Count_Cache vacc"
        ...
        String PIRSF_ACCID_KEY = "select tv._Term_key, tv.accId, tv.vocabName, tv.term"
                + " from VOC_Term_View tv, VOC_Annot_Count_Cache vacc"
        ...
        String OMIM_ACCID_KEY = "select tv._Term_key, tv.accId, tv.vocabName, tv.term"
                + " from VOC_Term_View tv, VOC_Annot_Count_Cache vacc"
        ...
        String OMIM_HUMAN_ACCID_KEY = "select tv._Term_key, tv.accId, '"+IndexConstants.OMIM_ORTH_TYPE_NAME+"' as vocabName, tv.term"
                + " from VOC_Term_View tv, VOC_Annot_Count_Cache vacc"
                + " where isObsolete != 1 and _Vocab_key = 44"
    ...
    /**
     * Generic Accession ID Gathering Algorithm.  It takes a SQL string with an assumed format, and
     * creates Lucene Documents from it.
     * 
     * @throws SQLException
     * @throws InterruptedException
     */
    
    private void doVocabAccessionID(String sql) throws SQLException, InterruptedException {
        ...
        while (!rs_acc_id.isAfterLast()) {
            ...
            new_vocab.setDisplay_type(hm.get(rs_acc_id.getString("vocabName")));
