 ...
 * ("Allele Name)
 * 
 * A single instance of the MarkerExactLuceneDocBuilder which is used throughout
 * to create its needed Lucene documents
 * 
...
public class MarkerExactGatherer extends AbstractGatherer {
    ...
    private void doLabels() throws SQLException, InterruptedException {
        ...
        String GENE_LABEL_EXACT = "select ml._Marker_key, "
                + "ml.label, ml.labelType,  ml.labelTypeName, ml._OrthologOrganism_key, "
                + "ml._Label_Status_key, ml._Label_key" + " from MRK_Label ml, MRK_Marker m"
        ...
        while (!rs_label.isAfterLast()) {
            ...
            if (dataType.equals(IndexConstants.MARKER_SYNOYNM) && rs_label.getString("_OrthologOrganism_key") != null ) {
            ...
            if (!rs_label.getString("_Label_Status_key").equals("1")) {
                ...
                // If we have an old bit of nomen, we need to create a custom type.
    ...
    /**
     ...
     " and al._Label_Status_key != 0";
            ...
            while (!rs.isAfterLast()) {
                ...
                markerExact.setUnique_key(rs.getString("_Marker_key")+rs.getString("label") + IndexConstants.MARKER_TYPE_NAME);
